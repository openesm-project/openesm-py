...
def test_process_raw_datasets_list_schema_error(mock_msg_warn, sample_raw_datasets_list):
...
def test_list_datasets_use_cache(mock_msg_info, mock_time, mock_get_cache_dir, temp_cache_dir, sample_raw_datasets_list):
    ...
    # Should use cache
    mock_msg_info.assert_called_with("Using cached dataset index (less than 24 hours old).")
...
@patch("openesm.list_datasets.msg_info")
def test_list_datasets_download_fresh(mock_msg_info, mock_download, mock_time, mock_get_cache_dir, temp_cache_dir, sample_raw_datasets_list):
...
@patch("openesm.list_datasets.download_with_progress")
@patch("openesm.list_datasets.msg_info")
def test_list_datasets_no_cache_file(mock_msg_info, mock_download, mock_get_cache_dir, temp_cache_dir, sample_raw_datasets_list):
...
@patch("openesm.list_datasets.download_with_progress")
@patch("openesm.list_datasets.msg_info")
def test_list_datasets_force_download(mock_msg_info, mock_download, mock_get_cache_dir, temp_cache_dir, sample_raw_datasets_list):
...
def test_process_features_string_conversion():
    ...
    test_cases = [
        ...
        {
            "dataset_id": "test1", 
            "features": [{"name": "mood"}, {"name": "stress"}]
        },
        # Single feature as list
        {
            "dataset_id": "test2",
            "features": [{"name": "mood", "type": "numeric"}]
        },
        # None/empty features
        {
            "dataset_id": "test3",
...
def test_list_datasets_integration_with_custom_cache_hours(temp_cache_dir, sample_raw_datasets_list):
    """Integration test with custom cache_hours parameter."""
    # This test doesn't use mocks to test the integration
    with patch("openesm.list_datasets.get_cache_dir") as mock_get_cache_dir, \
...
def test_process_features_with_string_fallback():
    ...
    test_cases = [
        {
            "dataset_id": "test1",
            "features": "simple string"  # This gets processed by process_specific_metadata
        }
    ]
...
def test_process_raw_datasets_list_polars_error_fallback(mock_dataframe, mock_msg_warn):
    ...
    mock_dataframe.side_effect = [
        ...
        pl.DataFrame([{"dataset_id": "test", "features": None}])  # Second call succeeds
    ]
        ...
        "datasets": [
            {
                "dataset_id": "0001",
                "first_author": "Smith",
                "unique_field_1": "value1"
            },
            {
                "dataset_id": "0002", 
                "first_author": "Jones",
                "unique_field_2": "value2"
            }
        ]
    }
